...
\ipolSetTitle{Intrisic Curve Interpolation}
...
\begin{figure}[h]
	\centering
	\subfloat[Euclidean interpolation]{
	...
	\subfloat[Intrinsic interpolation]{
	...
	\caption{Example of two possible choices of curve interpolation. Input curves are depicted in blue and red, and the interpolated curve at $t=0.5$ is depicted in green. \protect\subref{fig:image_interpolation:curve_interpolation:example:euclidean} Interpolation performed directly over the control points (Euclidean interpolation). \protect\subref{fig:image_interpolation:curve_interpolation:example:curvature} Interpolation performed only in intrinsic shape parameters. Note how the choice of the interpolation method can greatly affect the results.}
...
\begin{enumerate}
    \item \textbf{The curve representation}, which defines how the curve is described and manipulated:
        \begin{enumerate}
            \item How the input curve is expressed: poly-lines, splines, sampled curves, implicit surfaces or bitmaps \tcom{Agregar referencias}.
...
\begin{figure}[h]
	...
	\caption{Example of preprocessing of an input curve given by control points. 
		\protect\subref{fig:curve_interpolation:control_points:c1} Complete curve with sparse control points are represented as a blue line and interpolated points are shown as blue dots.
...
\clearpage
\input{appendix_b_numerical_computation}
\clearpage
